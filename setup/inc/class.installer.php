    ...
    function install($vars) {
        ...
        if(!$this->errors) {
            ...
            elseif(explode('.', db_version()) < explode('.', $this->getMySQLVersion()))
                $this->errors['db']=sprintf('Lagos State University Electronic HelpDesk requires MySQL %s or better!',$this->getMySQLVersion());
        ...
        if(!$this->errors) {
            ...
            $sql='INSERT INTO '.PREFIX.'email (`name`,`email`,`created`,`updated`) VALUES '
                    ...
                    .",('LASU E-HelpDesk Alerts','alerts@$domain',NOW(),NOW())"
        ...
        //Log a message.
        $msg="Congratulations Lagos State University basic installation completed!\n\nThank you for choosing Lagos  State University!";
        $sql='INSERT INTO '.PREFIX.'syslog SET created=NOW(), updated=NOW(), log_type="Debug" '
            .', title="Lagos State University Electronic HelpDesk installed!"'
            .', log='.db_input($msg)
